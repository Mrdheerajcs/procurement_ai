...
async def upload_pdf(
    ...
    if file.filename.endswith(".pdf"):
        scan_pdf(file_path, file.filename, category, sub_category)
    ...
    if file.filename.endswith(".docx"):
        scan_word_document(file_path, file.filename, category, sub_category)

        return {
            "success": True,
            "message": "Word document uploaded and scanned successfully",
            "file_path": file_path,
        }
...
def generate_tender(request: TenderRequest):
    ...
    suggested_title = request.requirement.title() + " Procurement - IT Department"
    ...
    df = execute_sql_query(sql, DB_URL)
    ...
    for _, row in df.iterrows():
        ...
        suggested_items.append(
            {
                ...
                "unit": "Nos",
                "estimated_rate": rate,
                "specification": row.get("specificationn", "N/A"),
        ...
        similarity = int(
            SequenceMatcher(None, requirement, search_text.lower()).ratio() * 100
        )
    ...
    LAST_GENERATED_TENDER = {
        "success": True,
        "project_name": (
            past_references[0]["project_name"] if past_references else ""
        ),
...
def download_word():
    ...
    doc.add_heading("DETAILED TENDER NOTICE", level=1)
    doc.add_paragraph("Technical Specification + BOQ + Approval Note")
    doc.add_heading("MATERIAL PURCHASE REQUEST", level=2)
    ...
    doc.add_paragraph(
        f"Total Estimated Value: "
        f"{LAST_GENERATED_TENDER.get('total_estimated_value', 0)}"
    )

    doc.add_heading("TECHNICAL SPECIFICATIONS", level=2)

    for item in LAST_GENERATED_TENDER.get("suggested_items", []):
        doc.add_paragraph(f"• {item.get('specification', '')}")
...
def download_pdf():
    ...
    elements.append(Paragraph("<b>DETAILED TENDER NOTICE</b>", styles["Title"]))
    elements.append(
        Paragraph(
    ...
    elements.append(Paragraph("<b>MATERIAL PURCHASE REQUEST</b>", styles["Heading2"]))
    ...
    elements.append(
        Paragraph(
            f"<b>Total Estimated Value:</b> "
            f"{LAST_GENERATED_TENDER.get('total_estimated_value', 0)}",
            styles["BodyText"],
        )
    )
    elements.append(Spacer(1, 15))
    elements.append(Paragraph("<b>TECHNICAL SPECIFICATIONS</b>", styles["Heading2"]))
